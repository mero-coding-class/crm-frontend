...
const FileUploader = ({ onFileUploadSuccess, token, label = "Upload Files", multiple = true, accept = "*/*" }) => {
  ...
  const handleUpload = async () => {
    ...
    try {
      for (const file of selectedFiles) {
        // Assuming api.uploadFile handles single file upload
  ...
  return (
    <div className="mb-4 p-4 border border-dashed border-gray-300 rounded-md bg-white"> {/* Replaced card-like styling */}
      ...
      >
        ...
        <p className="text-gray-600 mb-1">Drag & drop files here, or <span className="text-blue-600 font-medium">browse</span></p>
      ...
      {selectedFiles.length > 0 && (
        <div className="mt-4 border-t border-gray-200 pt-4">
          ...
          <button
            ...
            disabled={uploading}
            className={`mt-4 w-full bg-blue-600 text-white font-medium py-2 px-4 rounded-md hover:bg-blue-700 transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50 ${uploading ? 'opacity-50 cursor-not-allowed' : ''}`}
          >
